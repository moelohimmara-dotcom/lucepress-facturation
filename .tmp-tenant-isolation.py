...
# clients.list (doit voir seulement 1 client : "Client Tenant 3")
...
try:
    ...
    for c in clients: print(f"    - {c.get('companyName')} (tenantId: {c.get('tenantId')})")
except: print(f"[3] body: {b3[:200]}")

# users.list (doit voir seulement 1 user : admin tenant 3)
s4, b4 = get("users.list", cookie)
try:
    d = json.loads(b4); users = d["result"]["data"]["json"]
    print(f"[4] users.list -> {len(users)} users")
    for u in users: print(f"    - {u.get('email')} (tenantId: {u.get('tenantId')})")
except: print(f"[4] body: {b4[:200]}")

print("\nVERDICT:")
print("  ISOLATION MULTI-TENANT CONFIRMEE" if me and me.get('tenantId') == 3 else "A VERIFIER")
