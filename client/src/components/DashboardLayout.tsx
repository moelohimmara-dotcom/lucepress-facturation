...
export function DashboardLayoutContent({ children, sidebarWidth = DEFAULT_WIDTH, setSidebarWidth }: { children: React.ReactNode; sidebarWidth?: number; setSidebarWidth: (width: number) => void }) {
  ...
  return (
    <>
      {/* Lien d’évitement UNIQUE, et PREMIER élément focalisable du document :
          c’est ce qui lui permet de faire son travail (WCAG 2.4.1). Le second
          lien, rendu plus bas dans le DOM au même `z-index` et aux mêmes
          coordonnées de focus, recouvrait le premier (WCAG 2.2 « Focus Not
          Obscured ») et imposait deux tabulations pour une même destination. */}
      <a href="#main-content" className="lucepress-skip-link">Aller au contenu principal</a>
      <div className="relative" data-testid="sidebar-shell" ref={sidebarRef} style={{ "--sidebar-width": `${effectiveSidebarWidth}px` } as CSSProperties}>
        <Sidebar collapsible="icon" aria-label="Navigation principale" className="border-r-0 bg-sidebar text-sidebar-foreground" disableTransition={isResizing}>
          <SidebarHeader className={`${isCompact ? "min-h-[88px]" : "min-h-[112px]"} shrink-0 justify-center px-3 py-2`}>
            <div className="flex items-center gap-3">
              <button onClick={toggleSidebar} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.08] shadow-sm transition-colors hover:bg-white/15 focus-visible:ring-2 focus-visible:ring-sidebar-ring" aria-label="Réduire la navigation">
                <PanelLeft className="h-4 w-4" aria-hidden="true" />
              </button>
              {!isCollapsed && <div className="min-w-0 flex-1"><p className="font-editorial text-xl font-semibold leading-none tracking-tight">{LUCEPRES_PUBLIC_PROFILE.displayName}</p>{!isCompact && <p className="mt-1.5 text-xs font-bold uppercase tracking-[0.18em] text-sidebar-foreground/55">Solutions durables · opérations</p>}</div>}
              {!isCollapsed && <div className="flex shrink-0 items-center gap-0.5"><button type="button" onClick={toggleTheme} className="flex h-11 w-11 items-center justify-center rounded-xl text-sidebar-foreground/65 transition-colors hover:bg-white/10 hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring" aria-label={theme === "dark" ? "Activer le thème clair" : "Activer le thème sombre"} title={theme === "dark" ? "Activer le thème clair" : "Activer le thème sombre"}>{theme === "dark" ? <Sun className="h-4 w-4" aria-hidden="true" /> : <Moon className="h-4 w-4" aria-hidden="true" />}</button><button data-testid="sidebar-density-toggle" onClick={toggleDensity} className="flex h-11 w-11 items-center justify-center rounded-xl text-sidebar-foreground/65 transition-colors hover:bg-white/10 hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring" aria-label={isCompact ? "Passer au mode normal" : "Passer au mode compact"} title={isCompact ? "Passer au mode normal" : "Passer au mode compact"}>{isCompact ? <Maximize2 className="h-4 w-4" aria-hidden="true" /> : <Minimize2 className="h-4 w-4" aria-hidden="true" />}</button><button type="button" data-testid="sidebar-help-trigger" onClick={() => { setGuidanceStep(0); setShowSidebarHelp(true); }} className="flex h-11 w-11 items-center justify-center rounded-xl text-sidebar-foreground/65 transition-colors hover:bg-white/10 hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring" aria-label="Ouvrir le guide d’accueil" title="Ouvrir le guide d’accueil"><CircleHelp className="h-4 w-4" aria-hidden="true" /></button></div>}
            </div>
          </SidebarHeader>
          <SidebarContent className={`relative min-h-0 gap-0 overflow-y-auto ${isCompact ? "px-1 py-2" : "px-2 py-3"}`}>
            {visibleGroups.map((group, groupIndex) => <div key={group.label} className={groupIndex ? `${isCompact ? "mt-1" : "mt-4 border-t border-white/[0.08] pt-3"}` : ""}>
              {!isCollapsed && !isCompact && <p className="mb-2 px-3 text-xs font-extrabold uppercase tracking-[0.19em] text-sidebar-foreground/42">{group.label}</p>}
          ...
          <SidebarFooter className={`${isCompact ? "p-2" : "p-3"} shrink-0 border-t border-white/10`}>
            {!isCollapsed && <button type="button" data-testid="sidebar-ai-assistant" onClick={() => setLocation("/devis/nouveau?assistant=1")} aria-keyshortcuts="Alt+3" aria-label="Ouvrir l’assistant IA pour créer un devis" className={`${isCompact ? "mb-0.5 rounded-xl p-2.5" : "mb-1 rounded-2xl p-3.5"} w-full border border-white/10 bg-white/[0.06] text-left transition-colors hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-sidebar-ring`}><div className="flex gap-2.5"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-sidebar-primary" aria-hidden="true" /><div><p className="text-xs font-bold">Assistant IA</p>{!isCompact && <p className="mt-1 text-xs leading-4 text-sidebar-foreground/60">Transformez un besoin de chantier en devis à valider.</p>}</div></div></button>}
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex w-full items-center gap-3 rounded-xl p-2 text-left transition-colors hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-sidebar-ring">
                  ...
                  {!isCollapsed && <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{user?.name || "Utilisateur Lucepres"}</p>{!isCompact && <p className="mt-0.5 truncate text-xs text-sidebar-foreground/55">{user?.email || "Accès sécurisé"}</p>}</div>}
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-52"><DropdownMenuItem onClick={logout} className="cursor-pointer text-destructive focus:text-destructive"><LogOut className="mr-2 h-4 w-4" aria-hidden="true" />Se déconnecter</DropdownMenuItem></DropdownMenuContent>
            </DropdownMenu>
          </SidebarFooter>
        </Sidebar>
        <div
      ...
      <SidebarInset className="bg-background">
        <header className="sticky top-0 z-40 flex min-h-[66px] items-center justify-between gap-3 border-b border-border bg-background/90 px-4 py-2 backdrop-blur sm:px-6 lg:px-8"><div className="flex min-w-0 items-center gap-3">{isMobile && <SidebarTrigger className="h-11 w-11 shrink-0 rounded-xl border border-border bg-card" />}<div><p className="font-editorial text-lg font-semibold leading-none">{isMobile ? LUCEPRES_PUBLIC_PROFILE.displayName : activeMenuItem?.label ?? "Gestion"}</p><p className="mt-1 text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground">{isMobile ? activeMenuItem?.label ?? "Gestion" : "Recherche et accès rapides"}</p></div></div><div className="flex shrink-0 items-center gap-2"><button type="button" data-testid="workspace-search-trigger" onClick={() => setWorkspaceSearchOpen(true)} aria-label="Rechercher dans l’espace de travail" className="flex h-11 items-center gap-2 rounded-xl border border-border bg-card px-3 text-xs font-bold text-muted-foreground shadow-sm transition-colors hover:border-primary/35 hover:text-primary sm:min-w-56"><Search className="h-4 w-4 text-primary" aria-hidden="true" /><span className="hidden sm:inline">Rechercher…</span><CommandShortcut className="hidden text-xs sm:inline">Ctrl+K</CommandShortcut></button>{isMobile && <><button type="button" onClick={() => { setGuidanceStep(0); setShowSidebarHelp(true); }} className="flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-card text-primary shadow-sm" aria-label="Ouvrir le guide d’accueil"><CircleHelp className="h-4 w-4" aria-hidden="true" /></button><button type="button" onClick={toggleTheme} className="flex h-11 w-11 items-center justify-center rounded-xl border border-border bg-card text-primary shadow-sm" aria-label={theme === "dark" ? "Activer le thème clair" : "Activer le thème sombre"}>{theme === "dark" ? <Sun className="h-4 w-4" aria-hidden="true" /> : <Moon className="h-4 w-4" aria-hidden="true" />}</button></>}</div></header>
      ...
      <Dialog open={showSidebarHelp} onOpenChange={setSidebarHelpOpen}>
        <DialogContent data-testid="sidebar-help" showCloseButton={false} className="max-w-[calc(100%-2rem)] gap-0 overflow-hidden rounded-[1.6rem] border-sidebar-primary/35 bg-sidebar p-0 text-sidebar-foreground shadow-2xl sm:max-w-[30rem]">
          <div className="relative"><div className="absolute -right-10 -top-10 h-36 w-36 rounded-full bg-sidebar-primary/20 blur-2xl" /><div className="relative p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-sidebar-primary text-xs font-extrabold text-sidebar-primary-foreground">{guidanceStep + 1}/3</div><button type="button" onClick={dismissSidebarHelp} className="flex h-9 w-9 items-center justify-center rounded-xl text-sidebar-foreground/65 transition-colors hover:bg-white/10 hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring" aria-label="Fermer le guide d’accueil"><X className="h-4 w-4" aria-hidden="true" /></button></div><p className="mt-7 text-xs font-extrabold uppercase tracking-[0.2em] text-sidebar-primary">{guidance.eyebrow}</p><DialogTitle className="mt-2 font-editorial text-2xl font-semibold leading-tight text-sidebar-foreground">{guidance.title}</DialogTitle><DialogDescription className="mt-3 text-sm leading-6 text-sidebar-foreground/72">{guidance.body}</DialogDescription><div className="mt-5 flex flex-wrap gap-2">{guidance.highlights.map(highlight => <span key={highlight} className="rounded-full border border-white/10 bg-white/[0.08] px-3 py-1.5 text-xs font-bold text-sidebar-foreground/85">{highlight}</span>)}</div><div className="mt-7 flex items-center justify-between gap-3 border-t border-white/[0.1] pt-4"><div className="flex items-center gap-1.5" aria-label={`Étape ${guidanceStep + 1} sur ${sidebarGuidanceSteps.length}`}>{sidebarGuidanceSteps.map((step, index) => <span key={step.eyebrow} className={`h-1.5 rounded-full transition-all ${index <= guidanceStep ? "w-7 bg-sidebar-primary" : "w-2 bg-white/20"}`} />)}</div><Button type="button" size="sm" onClick={advanceGuidance} className="h-10 rounded-xl bg-sidebar-primary px-4 text-xs font-extrabold text-sidebar-primary-foreground hover:bg-sidebar-primary/90">{isFinalGuidanceStep ? <><Check className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />Terminer</> : <>Suivant<ArrowRight className="ml-1.5 h-3.5 w-3.5" aria-hidden="true" /></>}</Button></div></div></div>
        </DialogContent>
      </Dialog>
      <CommandDialog open={workspaceSearchOpen} onOpenChange={open => { if (open) setWorkspaceSearchOpen(true); else closeWorkspaceSearch(); }} title="Recherche globale" description="Recherchez un client, un devis, une facture ou une créance." className="max-w-[calc(100%-2rem)] rounded-2xl border-border bg-card sm:max-w-xl">
        ...
        <div className="border-b border-border/70 px-3 py-2">
          <div className="flex items-center justify-between gap-3">
            <button type="button" onClick={() => setWorkspaceSearchFiltersOpen(open => !open)} className="inline-flex items-center gap-2 text-xs font-extrabold text-primary hover:underline" aria-expanded={workspaceSearchFiltersOpen}><SlidersHorizontal className="h-3.5 w-3.5" aria-hidden="true" />Filtres avancés{activeWorkspaceFilterCount > 0 && <span className="rounded-full bg-primary px-1.5 py-0.5 text-xs text-primary-foreground">{activeWorkspaceFilterCount}</span>}</button>
            {activeWorkspaceFilterCount > 0 && <button type="button" onClick={resetWorkspaceSearchFilters} className="text-xs font-bold text-muted-foreground hover:text-primary">Réinitialiser</button>}
          </div>
          {workspaceSearchFiltersOpen && <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <label className="text-xs font-extrabold uppercase tracking-wide text-muted-foreground">Type<select aria-label="Filtrer par type" value={workspaceSearchFilters.kind || ""} onChange={event => updateWorkspaceSearchFilter("kind", event.target.value ? event.target.value as WorkspaceSearchFilters["kind"] : undefined)} className="mt-1 h-9 w-full rounded-lg border border-border bg-card px-2 text-xs font-semibold normal-case tracking-normal text-foreground"><option value="">Tous les résultats</option><option value="client">Clients</option><option value="devis">Devis</option><option value="facture">Factures</option><option value="creance">Créances</option></select></label>
            <label className="text-xs font-extrabold uppercase tracking-wide text-muted-foreground">Statut<select aria-label="Filtrer par statut" value={workspaceSearchFilters.status || ""} onChange={event => updateWorkspaceSearchFilter("status", event.target.value || undefined)} className="mt-1 h-9 w-full rounded-lg border border-border bg-card px-2 text-xs font-semibold normal-case tracking-normal text-foreground"><option value="">Tous les statuts</option><option value="brouillon">Brouillon</option><option value="a_envoyer">À envoyer</option><option value="envoye">Envoyé</option><option value="accepte">Accepté</option><option value="partiellement_paye">Partiellement payé</option><option value="paye">Payé</option><option value="en_retard">En retard</option><option value="a_traiter">À traiter</option><option value="contacte">Contacté</option><option value="a_rappeler">À rappeler</option></select></label>
            <label className="text-xs font-extrabold uppercase tracking-wide text-muted-foreground">Date de début<input aria-label="Date de début" type="date" value={workspaceSearchFilters.dateFrom || ""} onChange={event => updateWorkspaceSearchFilter("dateFrom", event.target.value || undefined)} className="mt-1 h-9 w-full rounded-lg border border-border bg-card px-2 text-xs font-semibold normal-case tracking-normal text-foreground" /></label>
            <label className="text-xs font-extrabold uppercase tracking-wide text-muted-foreground">Date de fin<input aria-label="Date de fin" type="date" value={workspaceSearchFilters.dateTo || ""} onChange={event => updateWorkspaceSearchFilter("dateTo", event.target.value || undefined)} className="mt-1 h-9 w-full rounded-lg border border-border bg-card px-2 text-xs font-semibold normal-case tracking-normal text-foreground" /></label>
            <label className="text-xs font-extrabold uppercase tracking-wide text-muted-foreground">Montant minimum<input aria-label="Montant minimum" type="number" min="0" step="1" placeholder="GNF" value={workspaceSearchFilters.amountMin ?? ""} onChange={event => updateWorkspaceSearchFilter("amountMin", event.target.value ? Number(event.target.value) : undefined)} className="mt-1 h-9 w-full rounded-lg border border-border bg-card px-2 text-xs font-semibold normal-case tracking-normal text-foreground" /></label>
            <label className="text-xs font-extrabold uppercase tracking-wide text-muted-foreground">Montant maximum<input aria-label="Montant maximum" type="number" min="0" step="1" placeholder="GNF" value={workspaceSearchFilters.amountMax ?? ""} onChange={event => updateWorkspaceSearchFilter("amountMax", event.target.value ? Number(event.target.value) : undefined)} className="mt-1 h-9 w-full rounded-lg border border-border bg-card px-2 text-xs font-semibold normal-case tracking-normal text-foreground" /></label>
            <label className="text-xs font-extrabold uppercase tracking-wide text-muted-foreground">Trier par<select aria-label="Trier les résultats" value={workspaceSearchFilters.sortBy || "relevance"} onChange={event => updateWorkspaceSearchFilter("sortBy", event.target.value as WorkspaceSearchFilters["sortBy"])} className="mt-1 h-9 w-full rounded-lg border border-border bg-card px-2 text-xs font-semibold normal-case tracking-normal text-foreground"><option value="relevance">Pertinence</option><option value="date">Date</option><option value="status">Statut</option><option value="amount">Montant</option></select></label>
            <label className="text-xs font-extrabold uppercase tracking-wide text-muted-foreground">Ordre<select aria-label="Ordre de tri" value={workspaceSearchFilters.sortDirection || "desc"} onChange={event => updateWorkspaceSearchFilter("sortDirection", event.target.value as WorkspaceSearchFilters["sortDirection"])} className="mt-1 h-9 w-full rounded-lg border border-border bg-card px-2 text-xs font-semibold normal-case tracking-normal text-foreground"><option value="desc">Décroissant</option><option value="asc">Croissant</option></select></label>
          </div>}
        </div>
        <CommandList className="max-h-[min(55vh,430px)] p-2">
          {workspaceSearchQuery.trim().length < 2 ? <CommandEmpty>Saisissez au moins deux caractères pour commencer.</CommandEmpty> : isWorkspaceSearching ? <p className="px-3 py-6 text-center text-sm text-muted-foreground">Recherche en cours…</p> : resultGroups.filter(group => group.results.length > 0).map(group => <CommandGroup key={group.kind} heading={group.label}>{group.results.map(result => <CommandItem key={`${result.kind}-${result.id}`} value={`${result.kind} ${result.title} ${result.subtitle}`} onSelect={() => selectWorkspaceResult(result.href)}><group.icon className="h-4 w-4 text-primary" aria-hidden="true" /><div className="min-w-0"><p className="truncate text-sm font-bold">{result.title}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{result.subtitle}</p></div><ArrowRight className="ml-auto h-4 w-4 text-primary/70" aria-hidden="true" /></CommandItem>)}</CommandGroup>)}
