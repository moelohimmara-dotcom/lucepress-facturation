...
import { ArrowUpRight, Bot, FilePlus2, ReceiptText, Send, Sparkles, WalletCards } from "lucide-react";
...
export default function Home() {
  ...
  const indicators = [
    ...
    { label: "En retard", value: isLoading ? "…" : String(dashboard?.counts.overdue ?? 0), caption: `${dashboard?.counts.invoicesToFollow ?? 0} facture(s) à suivre`, icon: ReceiptText, tone: "bg-[#f8ebcb] text-[#8c5b13]" },
  ];
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-7xl space-y-6 lg:space-y-8">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {indicators.map(({ label, value, caption, icon: Icon, tone }) => <article className="card-shadow rounded-2xl border border-border bg-card p-5" key={label}><div className="flex items-start justify-between"><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${tone}`}><Icon className="h-5 w-5" /></div><span className="font-mono text-xs text-muted-foreground">En direct</span></div><p className="mt-5 text-sm font-bold text-muted-foreground">{label}</p><p className="font-editorial mt-1 text-3xl font-semibold">{value}</p><p className="mt-2 text-xs text-muted-foreground">{caption}</p></article>)}
        </section>
