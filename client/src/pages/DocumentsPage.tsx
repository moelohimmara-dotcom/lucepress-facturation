...
import { ArrowUpRight, FilePlus2, FileText, ReceiptText, Search } from "lucide-react";
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-col gap-5 border-b border-border pb-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-primary">Gestion commerciale</p><h1 className="font-editorial mt-2 text-3xl font-semibold">{label}</h1><p className="mt-2 text-sm text-muted-foreground">Créez, suivez et faites évoluer vos {kind === "devis" ? "propositions commerciales" : "demandes de règlement"}.</p></div><Button onClick={() => setLocation(`/${kind}/nouveau`)} className="h-10 rounded-xl bg-primary font-bold text-primary-foreground shadow-lg shadow-primary/15"><FilePlus2 className="mr-2 h-4 w-4" />Nouveau {kind === "devis" ? "devis" : "facture"}</Button></header>
        <div className="mt-6 flex h-11 max-w-md items-center gap-2 rounded-xl border border-border bg-card px-3"><Search className="h-4 w-4 text-muted-foreground" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Rechercher un numéro ou un client" className="h-full min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" /></div>
        <section className="card-shadow mt-5 overflow-hidden rounded-2xl border border-border bg-card"><div className="hidden grid-cols-[1.1fr_1.5fr_1fr_1fr_auto] gap-4 border-b border-border bg-muted/50 px-6 py-3 text-[10px] font-extrabold uppercase tracking-[0.16em] text-muted-foreground md:grid"><span>Document</span><span>Client · Chantier</span><span>Statut</span><span className="text-right">Montant TTC</span><span /></div>{isLoading ? <div className="p-12 text-center text-sm text-muted-foreground">Chargement des documents…</div> : filtered.length === 0 ? <div className="flex min-h-64 flex-col items-center justify-center px-6 text-center"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-secondary text-primary"><Icon className="h-5 w-5" /></div><h2 className="mt-4 text-sm font-extrabold">Aucun {kind} pour le moment</h2><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Créez votre premier document pour le retrouver ici avec son statut et son montant.</p></div> : <div>{filtered.map(document => <button key={document.id} onClick={() => setLocation(`/documents/${document.id}`)} className="grid w-full gap-3 border-b border-border px-5 py-4 text-left transition-colors last:border-0 hover:bg-muted/40 md:grid-cols-[1.1fr_1.5fr_1fr_1fr_auto] md:items-center md:gap-4 md:px-6"><div><p className="font-mono text-xs font-bold text-primary">{document.number}</p><p className="mt-1 text-xs text-muted-foreground">Émis le {new Date(document.issueDate).toLocaleDateString("fr-GN")}</p></div><div><p className="text-sm font-extrabold">{document.clientName}</p><p className="mt-1 truncate text-xs text-muted-foreground">{document.projectName || "Sans chantier associé"}</p></div><div><Badge className={`w-fit border-0 px-2.5 py-1 text-[10px] font-extrabold capitalize ${statusStyle[document.status] || statusStyle.brouillon}`}>{humanStatus(document.status)}</Badge></div><p className="text-left font-mono text-sm font-bold md:text-right">{formatGnf(document.total)}</p><ArrowUpRight className="hidden h-4 w-4 text-muted-foreground md:block" /></button>)}</div>}</section>
      </div>
    </DashboardLayout>
  );
}
