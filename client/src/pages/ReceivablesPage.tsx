...
function InvoiceCard({ invoice, ownerName, selected, onToggleSelect, onMarkReminderHandled, reminderPending, onOpenTimeline, onManageFollowUp, onOpen }: { invoice: any; ownerName: string | null; selected: boolean; onToggleSelect: () => void; onMarkReminderHandled: () => void; reminderPending: boolean; onOpenTimeline: () => void; onManageFollowUp: () => void; onOpen: () => void }) {
  ...
  return <article className={`group flex flex-col gap-4 px-5 py-5 transition-colors sm:flex-row sm:items-center sm:justify-between sm:px-6 ${status.row} ${selected ? "ring-1 ring-inset ring-primary/40" : ""}`}>
    <div className="flex min-w-0 gap-3">
      <div className="pt-0.5"><input aria-label={`Sélectionner la facture ${invoice.number}`} type="checkbox" checked={selected} onChange={onToggleSelect} className="h-4 w-4 rounded border-primary/30 accent-primary" /></div>
