...
export default function ReceivablesPage() {
  ...
  const visibleInvoices = useMemo(() => (data?.invoices ?? []).filter(invoice => {
    ...
    const ownerMatch = ownerFilter === "all" || (ownerFilter === "unassigned" ? !invoice.collectionOwnerId : invoice.collectionOwnerId === Number(ownerFilter));
    return match && periodMatch && ownerMatch && (!overdueOnly || invoice.isOverdue) && (!reminderTodayOnly || isCollectionReminderToday(invoice.collectionReminderDate));
  }), [data, search, overdueOnly, ownerFilter, period, reminderTodayOnly]);
  ...
  const reminderLoads = useMemo(() => buildCollectionReminderLoad(data?.invoices ?? [], assignees), [assignees, data?.invoices]);
  ...
  const updateFollowUp = trpc.billing.collection.updateFollowUp.useMutation({
    onSuccess: (_result, input) => {
      ...
      if (location.includes("?facture=")) setLocation("/creances");
      toast.success(input.collectionOwnerId !== undefined ? "Responsable de recouvrement mis à jour." : input.collectionStatus === "contacte" ? "Rappel marqué comme traité." : "Statut de suivi mis à jour.");
    },
    onError: error => toast.error(error.message),
  });
  ...
  useEffect(() => {
    const invoiceId = Number(new URLSearchParams(location.split("?")[1] ?? "").get("facture"));
    if (!invoiceId || !data) return;
    const invoice = data.invoices.find(item => item.id === invoiceId);
    if (!invoice) { setLocation("/creances"); return; }
    setFollowUpInvoice({ id: invoice.id, number: invoice.number, clientId: invoice.clientId, clientName: invoice.clientName, collectionStatus: invoice.collectionStatus, collectionReminderDate: invoice.collectionReminderDate, collectionOwnerId: invoice.collectionOwnerId });
  ...
  return <DashboardLayout><div className="mx-auto max-w-7xl pb-10 stagger-rise">
    <header className="relative overflow-hidden rounded-[1.45rem] border border-primary/15 bg-primary/[0.035] px-5 py-6 sm:px-7 sm:py-7">
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex w-full max-w-sm flex-col gap-2"><div className="flex items-start gap-3 rounded-xl border border-amber-200/80 bg-amber-50/80 px-4 py-3 text-xs leading-5 text-amber-950 shadow-sm dark:border-amber-700/60 dark:bg-amber-950/45 dark:text-amber-100"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0 text-amber-700 dark:text-amber-300" /><span><strong className="block">Supervision humaine</strong> Les relances restent à préparer et valider avant tout envoi.</span></div>{canPilotCollection && <Button type="button" variant="outline" onClick={() => setReportDialogOpen(true)} className="min-h-11 rounded-xl border-primary/20 bg-card text-xs font-extrabold text-primary"><FileDown className="mr-2 h-4 w-4" />Rapport mensuel PDF</Button>}</div>
      </div>
    </header>
    ...
    {isLoading ? <div className="flex min-h-[50vh] items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : <>
      ...
      {reminderLoads.length > 0 && <section className="lucepress-panel mt-5 rounded-[1.35rem] p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="lucepress-kicker">Charge de suivi</p><h2 className="font-editorial mt-2 text-xl font-semibold">Rappels ouverts par responsable</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Le compteur inclut toutes les créances encore classées « À rappeler ».</p></div><BellRing className="h-5 w-5 text-rose-700 dark:text-rose-300" /></div><div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{reminderLoads.map(load => <button type="button" key={load.ownerId ?? "unassigned"} onClick={() => setOwnerFilter(load.ownerId === null ? "unassigned" : String(load.ownerId))} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-card px-3 py-3 text-left transition-colors hover:border-primary/35 hover:bg-primary/[0.025]"><span className="min-w-0 truncate text-xs font-bold">{load.ownerName}</span><span className={`flex h-8 min-w-8 items-center justify-center rounded-full px-2 text-xs font-extrabold ${load.reminderCount ? "bg-rose-100 text-rose-800 dark:bg-rose-950/70 dark:text-rose-200" : "bg-muted text-muted-foreground"}`}>{load.reminderCount}</span></button>)}</div></section>}
      {(reminderSummary.today > 0 || reminderSummary.soon > 0 || reminderSummary.overdue > 0) && <section className="mt-5 flex flex-col gap-4 rounded-[1.35rem] border border-rose-300/80 bg-rose-50 px-5 py-4 text-rose-950 shadow-[0_18px_38px_-30px_rgba(190,24,93,.75)] dark:border-rose-700/70 dark:bg-rose-950/35 dark:text-rose-100 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-rose-100 text-rose-700 dark:bg-rose-900/70 dark:text-rose-200"><BellRing className="h-5 w-5" /></div><div><p className="text-sm font-extrabold">Rappels à piloter</p><p className="mt-1 text-xs leading-5 text-rose-800 dark:text-rose-200">{reminderSummary.today > 0 && <><strong>{reminderSummary.today}</strong> aujourd’hui{reminderSummary.soon || reminderSummary.overdue ? " · " : ""}</>}{reminderSummary.soon > 0 && <><strong>{reminderSummary.soon}</strong> dans les 3 jours{reminderSummary.overdue ? " · " : ""}</>}{reminderSummary.overdue > 0 && <><strong>{reminderSummary.overdue}</strong> date{reminderSummary.overdue > 1 ? "s" : ""} dépassée{reminderSummary.overdue > 1 ? "s" : ""}</>}</p></div></div><Button type="button" onClick={() => { if (!reminderTodayOnly) toggleTodayReminderFilter(); }} className="min-h-11 shrink-0 rounded-xlxl bg-rose-700 text-xs font-extrabold text-white hover:bg-rose-800 dark:bg-rose-600 dark:hover:bg-rose-500"><BellRing className="mr-2 h-4 w-4" />Voir les rappels du jour</Button></section>}
      {hasExpiredPromises && <section className="mt-5 flex flex-col gap-4 rounded-[1.35rem] border border-red-400/80 bg-red-50 px-5 py-4 text-red-950 shadow-[0_18px_38px_-30px_rgba(185,28,28,.8)] dark:border-red-600/70 dark:bg-red-950/45 dark:text-red-100 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-red-100 text-red-700 dark:bg-red-900/70 dark:text-red-200"><AlertTriangle className="h-5 w-5" /></div><div><p className="text-sm font-extrabold">Promesse de paiement dépassée</p><p className="mt-1 text-xs leading-5 text-red-800 dark:text-red-200">{summary?.expiredPromiseCount} facture{(summary?.expiredPromiseCount ?? 0) > 1 ? "s" : ""} concernée{(summary?.expiredPromiseCount ?? 0) > 1 ? "s" : ""} · <strong>{formatGnf(summary?.expiredPromiseTotal ?? 0)}</strong> à traiter en priorité.</p></div></div><Button onClick={() => { setOverdueOnly(true); }} className="min-h-11 shrink-0 rounded-xlxl bg-red-700 text-xs font-extrabold text-white hover:bg-red-800 dark:bg-red-600 dark:hover:bg-red-500"><ArrowUpRight className="mr-2 h-4 w-4" />Voir les retards</Button></section>}

      <section className="lucepress-panel mt-6 overflow-hidden rounded-[1.4rem]">
        <div className="flex flex-col gap-2 border-b border-border bg-primary/[0.018] px-5 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-muted-foreground">Responsable</p><p className="mt-1 text-xs text-muted-foreground">Isolez les créances attribuées à une personne.</p></div><label className="flex items-center gap-2"><UsersRound className="h-3.5 w-3.5 text-primary" /><span className="sr-only">Filtrer par responsable</span><select aria-label="Filtrer les créances par responsable" value={ownerFilter} onChange={event => setOwnerFilter(event.target.value)} className="lucepress-field min-h-11 min-w-52 rounded-xl px-3 text-sm"><option value="all">Tous les responsables</option><option value="unassigned">Sans responsable</option>{assignees.map(person => <option key={person.id} value={person.id}>{person.name || person.email || `Utilisateur ${person.id}`}</option>)}</select></label></div>
        <div className="border-b border-border px-5 py-5 sm:px-6"><div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div><p className="lucepress-kicker">File de traitement</p><h2 className="font-editorial mt-2 text-2xl font-semibold">Factures à encaisser</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Les promesses dépassées, rappels et retards sont affichés avant les autres échéances.</p></div><div className="flex flex-col gap-2 sm:flex-row"><label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input aria-label="Rechercher une créance" value={search} onChange={event => setSearch(event.target.value)} className="lucepress-field min-h-11 w-full rounded-xl py-2 pr-3 pl-9 text-sm sm:w-64" placeholder="Facture, client, chantier…" /></label><Button variant={overdueOnly ? "default" : "outline"} onClick={() => setOverdueOnly(value => !value)} className="min-h-11 rounded-xl text-xs font-extrabold"><AlertTriangle className="mr-2 h-3.5 w-3.5" />{overdueOnly ? "Retards affichés" : "Prioriser les retards"}</Button></div></div><div className="mt-5 flex flex-col gap-3 border-t border-border pt-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><p className="shrink-0 text-[10px] font-extrabold uppercase tracking-[0.16em] text-muted-foreground">Période de retard</p><div className="flex flex-wrap gap-2" role="group" aria-label="Période des créances">{periods.map(item => <button type="button" key={item.value} onClick={() => setPeriod(item.value)} className={`min-h-11 rounded-xl border px-3 text-xs font-extrabold transition-colors ${!reminderTodayOnly && period === item.value ? "border-primary bg-primary text-primary-foreground shadow-sm" : "border-border bg-card text-muted-foreground hover:border-primary/30 hover:bg-primary/[0.03]"}`}>{item.label}</button>)}<button type="button" aria-pressed={reminderTodayOnly} onClick={toggleTodayReminderFilter} className={`min-h-11 rounded-xl border px-3 text-xs font-extrabold transition-colors ${reminderTodayOnly ? "border-rose-700 bg-rose-700 text-white shadow-sm dark:border-rose-500 dark:bg-rose-600" : "border-rose-200 bg-rose-50 text-rose-800 hover:border-rose-400 dark:border-rose-900/70 dark:bg-rose-950/35 dark:text-rose-200"}`}><BellRing className="mr-1 inline h-3.5 w-3.5" />Rappels du jour</button></div></div><Button type="button" variant="outline" onClick={exportVisibleInvoices} disabled={!visibleInvoices.length} className="min-h-11 shrink-0 rounded-xlxl border-primary/20 bg-card text-xs font-extrabold text-primary"><Download className="mr-1.5 h-3.5 w-3.5" />Exporter la file filtrée</Button></div></div>
        {visibleSelectedInvoiceIds.length > 0 && <div className="flex flex-col gap-3 border-b border-primary/15 bg-primary/[0.045] px-5 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div className="flex items-center gap-2"><UsersRound className="h-4 w-4 text-primary" /><p className="text-xs font-bold"><strong>{visibleSelectedInvoiceIds.length}</strong> créance{visibleSelectedInvoiceIds.length > 1 ? "s" : ""} sélectionnée{visibleSelectedInvoiceIds.length > 1 ? "s" : ""} · maximum {BATCH_REMINDER_LIMIT} par lot</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="ghost" onClick={() => setSelectedInvoiceIds(current => current.filter(id => !visibleSelectedInvoiceIds.includes(id)))} className="min-h-11 px-3 text-xs font-bold text-muted-foreground"><X className="mr-1 h-3.5 w-3.5" />Effacer</Button>{canPilotCollection && <Button type="button" variant="outline" onClick={openReassignDialog} className="min-h-11 rounded-xl border-primary/20 bg-card text-xs font-extrabold text-primary"><UserRoundCheck className="mr-1.5 h-3.5 w-3.5" />Réattribuer</Button>}<Button type="button" onClick={openBatchDialog} disabled={!visibleSelectedOverdueInvoiceIds.length} className="h-9 rounded-lg bg-primary text-xs font-extrabold text-primary-foreground"><Sparkles className="mr-1.5 h-3.5 w-3.5" />Préparer les brouillons</Button></div></div>}
        {visibleInvoices.length ? <div className="divide-y divide-border">{visibleInvoices.map(invoice => <InvoiceCard key={invoice.id} invoice={invoice} ownerName={invoice.collectionOwnerId ? assigneeNames.get(invoice.collectionOwnerId) ?? "Responsable indisponible" : null} selected={visibleSelectedInvoiceIds.includes(invoice.id)} onToggleSelect={() => toggleInvoiceSelection(invoice.id)} onMarkReminderHandled={() => updateFollowUp.mutate({ documentId: invoice.id, collectionStatus: "contacte" })} reminderPending={updateFollowUp.isPending} onOpenTimeline={() => setTimelineClient({ id: invoice.clientId, name: invoice.clientName })} onManageFollowUp={() => setFollowUpInvoice(invoice)} onOpen={() => setLocation(`/documents/${invoice.id}`)} />)}</div> : <EmptyState icon={WalletCards} title="Aucune créance ne correspond à ces critères." description={`Filtre actif : ${periodLabel}. Les factures réglées intégralement ne figurent pas dans cette liste.`} />}
      </section>
    </>}
...
function InvoiceCard({ invoice, ownerName, selected, onToggleSelect, onMarkReminderHandled, reminderPending, onOpenTimeline, onManageFollowUp, onOpen }: { invoice: any; ownerName: string | null; selected: boolean; onToggleSelect: () => void; onMarkReminderHandled: () => void; reminderPending: boolean; onOpenTimeline: () => void; onManageFollowUp: () => void; onOpen: () => void }) {
  ...
  return <article className={`group flex flex-col gap-4 px-5 py-5 transition-colors sm:flex-row sm:items-center sm:justify-between sm:px-6 ${status.row} ${selected ? "ring-1 ring-inset ring-primary/40" : ""}`}>
    <div className="flex min-w-0 gap-3">
      <div className="pt-0.5"><input aria-label={`Sélectionner la facture ${invoice.number}`} type="checkbox" checked={selected} onChange={onToggleSelect} className="h-5 w-5 rounded border-primary/30 accent-primary" /></div>
      <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-mono text-sm font-extrabold text-primary">{invoice.number}</p><span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${status.badge}`}>{status.label}</span><span className="rounded-full bg-secondary px-2.5 py-1 text-[10px] font-extrabold text-primary">{collectionFollowUpLabels[followUpStatus]}</span></div>
        <p className="mt-3 text-sm font-extrabold">{invoice.clientName}</p><p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground"><span>{invoice.projectName ?? "Sans chantier rattaché"}</span><span className="hidden sm:inline">·</span><span className="inline-flex items-center gap-1"><CalendarClock className="h-3.5 w-3.5" />Échéance {formatDate(invoice.dueDate)}</span></p><p className="mt-2 inline-flex items-center gap-1.5 text-[11px] font-bold text-muted-foreground"><UserRoundCheck className="h-3.5 w-3.5 text-primary" />{ownerName || "Responsable non attribué"}</p>
        {followUpStatus === "a_rappeler" && invoice.collectionReminderDate && <div className={`mt-2 flex flex-wrap items-center gap-2 rounded-lg px-2.5 py-1.5 text-[11px] font-extrabold ${reminderStyle}`}><span className="inline-flex items-center gap-1.5"><BellRing className="h-3.5 w-3.5" />{reminderLabel} · {formatDate(invoice.collectionReminderDate)}</span><Button type="button" variant="ghost" size="sm" disabled={reminderPending} onClick={onMarkReminderHandled} className="min-h-11 rounded-xl bg-white/45 px-3 text-xs font-extrabold text-current hover:bg-white/70 dark:bg-black/15 dark:hover:bg-black/30"><CheckCircle2 className="mr-1 h-3.5 w-3.5" />{reminderPending ? "Mise à jour…" : "Marquer traité"}</Button></div>}
        {invoice.paymentPromise && <p className={`mt-3 inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-bold ${invoice.isPaymentPromiseOverdue ? "bg-red-200 text-red-950 dark:bg-red-900/65 dark:text-red-100" : "bg-primary/[0.07] text-primary"}`}><CalendarClock className="h-3.5 w-3.5" />Promesse déclarée : {formatDate(invoice.paymentPromise.promisedDate)}</p>}
      </div>
    </div>
    <div className="flex items-center justify-between gap-3 sm:justify-end"><div className="text-right"><p className={`font-mono text-lg font-extrabold ${priority === "promise" ? "text-red-700 dark:text-red-300" : priority === "overdue" ? "text-amber-800 dark:text-amber-200" : "text-primary"}`}>{formatGnf(invoice.balanceDue)}</p><p className="mt-1 text-[11px] text-muted-foreground">sur {formatGnf(invoice.total)} · encaissé {formatGnf(invoice.paidAmount)}</p></div><div className="flex gap-1"><Button aria-label={`Gérer le suivi de ${invoice.number}`} variant="ghost" size="icon" onClick={onManageFollowUp} className="h-11 w-11 rounded-xl text-primary hover:bg-primary/[0.08]"><ClipboardCheck className="h-4 w-4" /></Button><Button aria-label={`Voir l’historique de ${invoice.clientName}`} variant="ghost" size="icon" onClick={onOpenTimeline} className="h-11 w-11 rounded-xl text-primary hover:bg-primary/[0.08]"><History className="h-4 w-4" /></Button><Button aria-label={`Ouvrir la facture ${invoice.number}`} variant="outline" size="icon" onClick={onOpen} className="h-10 w-10 rounded-xl border-primary/20 bg-card text-primary transition-transform group-hover:-translate-y-0.5"><ExternalLink className="h-4 w-4" /></Button></div></div>
  </article>;
}
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        ...
        {hasDrafts ? (
          <div className="space-y-3 py-2">
              ...
              return (
                <details key={draft.documentId} className="rounded-xl border border-border bg-card p-4" open={drafts.length === 1}>
                  ...
                  <div className="mt-4 border-t border-border pt-4 text-sm leading-6">
                    ...
                    <Button type="button" variant="outline" onClick={() => onCopy(draft)} className="mt-4 min-h-11 rounded-xl border-border text-xs font-bold">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-3">
              ...
              <Button
                ...
                className="min-h-11 rounded-xl bg-primary font-bold text-primary-foreground"
              >
          ...
          <div className="grid gap-4 py-3">
            ...
            <Button type="button" onClick={onPrepare} disabled={!selectedCount || pending} className="min-h-11 rounded-xl bg-primary font-bold text-primary-foreground">
              <Mail className="mr-2 h-4 w-4" />
...
function CollectionMonthlyReportDialog({ open, month, pending, onOpenChange, onMonthChange, onDownload }: { open: boolean; month: string; pending: boolean; onOpenChange: (open: boolean) => void; onMonthChange: (month: string) => void; onDownload: () => void }) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle className="flex items-center gap-2"><FileDown className="h-4 w-4 text-primary" />Rapport mensuel PDF</DialogTitle><DialogDescription>Téléchargez une synthèse interne de l’encours actuel et des activités de recouvrement enregistrées pendant le mois choisi.</DialogDescription></DialogHeader><div className="grid gap-4 py-3"><label className="grid gap-1.5 text-xs font-extrabold">Mois du rapport<input aria-label="Mois du rapport de recouvrement" type="month" value={month} onChange={event => onMonthChange(event.target.value)} className="lucepress-field min-h-11 rounded-xl px-3 text-sm" /></label><div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-950 dark:border-amber-700/60 dark:bg-amber-950/40 dark:text-amber-100"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0 text-amber-700 dark:text-amber-300" /><span><strong className="block">Rapport de pilotage interne</strong> Il contient les actions tracées, les paiements du mois et la répartition du suivi des créances. Les relances y restent des brouillons soumis à validation.</span></div><Button type="button" onClick={onDownload} disabled={!month || pending} className="min-h-11 rounded-xl bg-primary font-bold text-primary-foreground"><Download className="mr-2 h-4 w-4" />{pending ? "Génération du rapport…" : "Générer et télécharger"}</Button></div></DialogContent></Dialog>;
}

function ToneButton({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) { return <button type="button" onClick={onClick} className={`min-h-11 rounded-xl border px-3 text-xs font-bold ${selected ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground hover:bg-muted"}`}>{children}</button>; }
