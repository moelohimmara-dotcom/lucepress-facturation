...
vi.mock("./_core/llm", () => ({
  ...
  invokeLLMWithFallback: async (_params, candidates) => ({ choices: [{ message: { content: JSON.stringify({ companyName: "Bati Guinée", contactName: "Mamadou Diallo", email: "contact@batiguinee.example", phone: "+224 600 11 22 33", address: "Conakry, Ratoma", taxId: "NIF-2026", notes: "Projet de forage à étudier", missingFields: [] }) } }], model: candidates?.[0] ?? "gpt-5-mini" }),
...
describe("assistant.extractClient", () => {
  it("retourne une fiche client structurée qui reste à relire avant enregistrement", async () => {
    const ctx = { user: { id: 1, openId: "admin-test", name: "Admin", email: "admin@example.com", loginMethod: "manus", role: "admin", createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() }, tenantId: 1, req: {} as TrpcContext["req"], res: {} as TrpcContext["res"] } as TrpcContext;
