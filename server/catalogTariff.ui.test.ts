...
vi.mock("@/lib/trpc", async () => {
  ...
  return { trpc: {
    useUtils: () => ({ billing: { clients: { list: { invalidate: state.noOp } }, projects: { list: { invalidate: state.noOp } }, services: { list: { invalidate: state.noOp, setData: (_input: unknown, updater: (current: typeof state.catalog) => typeof state.catalog) => state.setCatalog((current: typeof state.catalog) => updater(current)) }, priceHistory: { invalidate: state.noOp } } } }),
    billing: {
...
afterEach(() => { cleanup(); Object.assign(state.catalog[0], { defaultUnitPrice: 0, defaultTaxRate: 0 }); state.history = []; });
...
describe("répertoire Prestations", () => {
  ...
  it("affiche les révisions tarifaires de la prestation ouverte", () => {
    state.history = [{ id: 5, previousUnitPrice: 200000, nextUnitPrice: 250000, previousTaxRate: 0, nextTaxRate: 18, changedByName: "Responsable Lucepres", createdAt: new Date("2026-08-26") }];
    render(createElement(CatalogPage, { kind: "prestations" }));
    fireEvent.click(screen.getByRole("button", { name: "Tarif" }));
    expect(screen.getByText("Historique des révisions")).toBeTruthy();
    expect(document.body.textContent).toMatch(/200.*000.*GNF.*250.*000.*GNF/);
    expect(screen.getByText(/Responsable Lucepres/)).toBeTruthy();
  });
});
