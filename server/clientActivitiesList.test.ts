...
describe("billing.clients.activities.list", () => {
  it("retourne l’historique filtré par fiche client", async () => {
    const ctx = { user: { id: 1, openId: "admin-history", name: "Admin", email: "admin@example.com", loginMethod: "manus", role: "admin", createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() }, tenantId: 1, req: {} as TrpcContext["req"], res: {} as TrpcContext["res"] } as TrpcContext;
