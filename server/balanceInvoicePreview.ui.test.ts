...
vi.mock("@/lib/trpc", () => ({ trpc: { useUtils: () => ({ billing: { documents: { get: { invalidate: noOp }, list: { invalidate: noOp } }, dashboard: { invalidate: noOp } } }), billing: { documents: { get: { useQuery: () => ({ data: depositInvoice, isLoading: false }) }, createDepositInvoice: { useMutation: () => ({ mutate: noOp, isPending: false }) }, createBalanceInvoice: { useMutation: ({ onSuccess }: any) => ({ isPending: false, mutate: (input: unknown) => { state.createBalanceInvoice(input); onSuccess({ id: 32, number: "FAC-2026-0032", existing: false }); } }) } }, settings: { get: { useQuery: () => ({ data: {} }) } }, payments: { create: { useMutation: () => ({ mutate: noOp, isPending: false }) } } } } }));
