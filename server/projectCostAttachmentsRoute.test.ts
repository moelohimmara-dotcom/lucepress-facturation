...
describe("route de justificatifs de coûts", () => {
  it("refuse un dépôt non administrateur", async () => { registerRoute(); mocks.authenticateRequest.mockResolvedValueOnce({ id: 4, role: "user" }); const res = response(); await handler(request(), res); expect(res.statusCode).toBe(401); });
  it("refuse un coût introuvable", async () => { registerRoute(); mocks.authenticateRequest.mockResolvedValueOnce({ id: 4, role: "admin" }); mocks.getProjectCostById.mockResolvedValueOnce(null); const res = response(); await handler(request(), res); expect(res.statusCode).toBe(400); });
  it("stocke uniquement les métadonnées après un dépôt valide", async () => { registerRoute(); mocks.authenticateRequest.mockResolvedValueOnce({ id: 4, role: "admin" }); mocks.getProjectCostById.mockResolvedValueOnce({ id: 8, projectId: 2 }); mocks.storagePut.mockResolvedValueOnce({ key: "project-cost-attachments/2/8/facture.pdf", url: "/manus-storage/project-cost-attachments/2/8/facture.pdf" }); mocks.createProjectCostAttachment.mockResolvedValueOnce({ id: 9 }); const res = response(); await handler(request(), res); expect(res.statusCode).toBe(201); expect(mocks.createProjectCostAttachment).toHaveBeenCalledWith(expect.objectContaining({ projectCostId: 8, createdById: 4, contentType: "application/pdf" })); });
});
