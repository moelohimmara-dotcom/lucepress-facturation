...
describe("route de justificatifs de coûts", () => {
  it("refuse un dépôt non administrateur", async () => { registerRoute(); mocks.authenticateRequest.mockResolvedValueOnce({ id: 4, role: "user", tenantId: 1 }); const res = response(); await handler(request(), res); expect(res.statusCode).toBe(401); });
