...
describe("billing.clients.update", () => {
  it("transmet les coordonnées modifiées au service sécurisé", async () => {
    const ctx = { user: { id: 1, openId: "admin-update", name: "Admin", email: "admin@example.com", loginMethod: "manus", role: "admin", createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() }, tenantId: 1, req: {} as TrpcContext["req"], res: {} as TrpcContext["res"] } as TrpcContext;
