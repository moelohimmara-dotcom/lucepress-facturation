...
vi.mock("@/lib/trpc", () => ({ trpc: { useUtils: () => ({ billing: { documents: { get: { invalidate: noOp }, list: { invalidate: noOp } }, dashboard: { invalidate: noOp } } }), billing: { documents: { get: { useQuery: () => ({ data: document, isLoading: false }) }, createDepositInvoice: { useMutation: () => ({ mutate: noOp, isPending: false }) }, createBalanceInvoice: { useMutation: () => ({ mutate: noOp, isPending: false }) }, sendByEmail: { useMutation: () => ({ mutate: noOp, isPending: false }) } }, settings: { get: { useQuery: () => ({ data: company }) } }, payments: { create: { useMutation: () => ({ mutate: noOp, isPending: false }) } }, mailStatus: { useQuery: () => ({ data: { smtpConfigured: true } }) } } } }));
