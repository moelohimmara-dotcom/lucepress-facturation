  ...
  trpc: {
    ...
    billing: {
      ...
      documents: {
        ...
        update: { useMutation: () => ({ mutate: noOp, isPending: false }) }, createDepositInvoice: { useMutation: () => ({ mutate: noOp, isPending: false }) }, createBalanceInvoice: { useMutation: () => ({ mutate: noOp, isPending: false }) }, sendByEmail: { useMutation: () => ({ mutate: noOp, isPending: false }) }, list: { useQuery: () => ({ data: [] }) },
      },
