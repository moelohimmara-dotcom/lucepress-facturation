...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    ...
    billing: {
      clients: { list: { useQuery: () => ({ data: [{ id: 8, companyName: "Client test", contactName: null, email: null, phone: null, address: null }] }) } },
      ...
      documents: {
        ...
        update: { useMutation: () => ({ mutate: noOp, isPending: false }) }, list: { useQuery: () => ({ data: [] }) },
      },
...
describe("éditeur de devis multi-services", () => {
  it("applique un modèle, modifie les lignes, enregistre l’échéancier et le restitue dans l’aperçu", async () => {
    ...
    expect(state.created).toHaveBeenCalledWith(expect.objectContaining({ depositPercent: 30, depositDueDate: "2026-08-31", balanceDueDate: "2026-09-26", lines: expect.arrayContaining([expect.objectContaining({ description: "Étude hydraulique", quantity: 2 })]) }));
    unmount();
